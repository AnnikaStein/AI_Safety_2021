...
if do_ptetaflavloss:
    ...
    for i in range(1, prev_epochs+1):
        checkpoint = torch.load(f'/hpcwork/um106329/may_21/saved_models/_ptetaflavloss{fl_text}_{NUM_DATASETS}_{default}_{n_samples}/model_{i}_epochs_v10_GPU_weighted_ptetaflavloss{fl_text}_{NUM_DATASETS}_datasets_with_default_{default}_{n_samples}.pt', map_location=torch.device(device))
...
if do_flatptetaflavloss:
    ...
    for i in range(1, prev_epochs+1):
        checkpoint = torch.load(f'/hpcwork/um106329/may_21/saved_models/_flatptetaflavloss{fl_text}_{NUM_DATASETS}_{default}_{n_samples}/model_{i}_epochs_v10_GPU_weighted_flatptetaflavloss{fl_text}_{NUM_DATASETS}_datasets_with_default_{default}_{n_samples}.pt', map_location=torch.device(device))
...
plt.ylabel('loss')
plt.legend()
plt.savefig(f'/home/um106329/aisafety/may_21/evaluate/loss_plots/{NUM_DATASETS}_files_{prev_epochs}_epochs_train_history_wm{weighting_method}{fl_text}_default_{default}_{n_samples}_samples.png', bbox_inches='tight', dpi=400, facecolor='w', transparent=False)
