...
if compare == False:
    ...
    if weighting_method == '_all':
        ...
        fpr,tpr,thresholds = metrics.roc_curve([(1 if test_targets[i]==0 else 0) for i in range(len(test_targets))],predictions_new_flat[:,0])
        ax1.plot(fpr,tpr)
        ...
        fpr,tpr,thresholds = metrics.roc_curve([(1 if test_targets[i]==1 else 0) for i in range(len(test_targets))],predictions_new_flat[:,1])
        ax2.plot(fpr,tpr)
        ...
        fpr,tpr,thresholds = metrics.roc_curve([(1 if test_targets[i]==2 else 0) for i in range(len(test_targets))],predictions_new_flat[:,2])
        ax3.plot(fpr,tpr)
        ...
        fpr,tpr,thresholds = metrics.roc_curve([(1 if test_targets[i]==3 else 0) for i in range(len(test_targets))],predictions_new_flat[:,3])
        ax4.plot(fpr,tpr)
    ...
    if weighting_method == '_flatptetaflavloss':
        fig = plt.figure(figsize=[15,15],num=40)
        fpr,tpr,thresholds = metrics.roc_curve([(1 if BvsUDSG_targets[i]==0 or BvsUDSG_targets[i]==1  else 0) for i in range(len(BvsUDSG_targets))],BvsUDSG_predictions_new_flat[:,0]+BvsUDSG_predictions_new_flat[:,1])
        plt.plot(fpr,tpr)
    ...
    for i in epochs:
        ...
        checkpoint = torch.load(f'/hpcwork/um106329/may_21/saved_models/{weighting_method}_{NUM_DATASETS}_{default}_{n_samples}/model_{i}_epochs_v10_GPU_weighted{weighting_method}_{NUM_DATASETS}_datasets_with_default_{default}_{n_samples}.pt', map_location=torch.device(device))
        model.load_state_dict(checkpoint["model_state_dict"])
    ...
    fig.savefig(f'/home/um106329/aisafety/may_21/evaluate/roc_curves/roc_BvL_weighting_method{weighting_method}_compare_at_epochs_{epochs}_{len_test}_jets_training_{NUM_DATASETS}_{default}_{n_samples}.png', bbox_inches='tight', dpi=400)
