...
def auc_ranking(deepcsv_input):
    ...
    return [display_names[deepcsv_input], auc_bvl, auc_bvc]
...
def auc_ranking_parallel_inputs_ready(deepcsv_input,inputs):
    #print(inputs)
    inputs = inputs.transpose()
    inputs = np.c_[inputs, flav]
...
if mode == 'raw':
    ...
    # Step 2: `pool.map` the function
    outs = [pool.map(auc_ranking_parallel, [i for i in range(start, end+1)])][0]
    print(outs)
    ...
    list_auc_bvc = [outs[i][2] for i in range(len(outs))]
...
def apply_noise(variable=0,magn=[param],offset=[0]):
    ...
    for s in range(0, NUM_DATASETS):
        ...
        for i, m in enumerate(magn):
            noise = torch.Tensor(np.random.normal(offset,m,len(all_inputs)))
            ...
            if s > 0:
                ...
                if variable in range(67):
                    defaults = abs(scalers[variable].inverse_transform(all_inputs) + 999) < 300   # "floating point error" --> allow some error margin
                    if np.sum(defaults) != 0:
                        xadv[defaults] = scalers[variable].inverse_transform(all_inputs)[defaults]
                '''        
...
if mode == 'noise':
    ...
    inputs = np.c_[noise_inputs, flav]
    ...
    # Step 2: `pool.map` the function
    outs = [pool.map(apply_noise, [i for i in range(start, end+1)])][0]
    print(outs)
...
def fgsm_attack(epsilon=param,sample=None,targets=None,reduced=True, scalers=None):
...
def compare_inputs(prop=0,epsilon=[param],minimum=None,maximum=None,reduced=True):
    xmagn = []
    for s in range(0, NUM_DATASETS):
        scalers = torch.load(scalers_file_paths[s])
        ...
        for i, m in enumerate(epsilon):
            if s > 0:
                xadv = np.concatenate((xmagn[i], scalers[prop].inverse_transform(fgsm_attack(epsilon[i],all_inputs,all_targets,reduced=reduced, scalers=scalers)[:,prop].cpu())))
                integervars = [59,63,64,65,66]
    ...
    del xmagn
    gc.collect()
    return auc_ranking_parallel_inputs_ready(prop,inputs)
...
if mode == 'FGSM':
    ...
    # Step 2: `pool.map` the function
    outs = [pool.map(compare_inputs, [i for i in range(start, end+1)])][0]
    print(outs)
    ...
    outs = [compare_inputs(i) for i in range(start, end+1)]
